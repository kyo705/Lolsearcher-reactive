...
public class MatchServiceTest {
    ...
    @Mock
    private FacadeMatchRecordProducerService facadeRecordProducerService;

    private MatchService matchService;

    @BeforeEach
    public void setup(){
        matchService = new MatchService(facadeRecordProducerService, riotGamesApi);
    }
    ...
    public void getRecentMatchTestWithSuccess(){
        ...
        StepVerifier.create(matchService.getRecentMatchDto(request))
                .consumeNextWith(matchDto ->
                        assertThat(matchDto.getQueueId()).isEqualTo(GameType.SOLO_RANK_MODE.getQueueId()))
                .consumeNextWith(matchDto ->
                        assertThat(matchDto.getQueueId()).isEqualTo(GameType.FLEX_RANK_MODE.getQueueId()))
                .consumeNextWith(matchDto ->
                        assertThat(matchDto.getQueueId()).isEqualTo(GameType.NORMAL_MODE.getQueueId())).expectComplete()
                .verify();
    ...
    public void getRecentMatchTestWithWebClientException(){
        ...
        StepVerifier.create(matchService.getRecentMatchDto(request))
                .consumeNextWith(matchDto ->
                    assertThat(matchDto.getQueueId()).isEqualTo(GameType.SOLO_RANK_MODE.getQueueId()))
                .consumeNextWith(matchDto ->
                        assertThat(matchDto.getQueueId()).isEqualTo(GameType.NORMAL_MODE.getQueueId()))
                .verifyComplete();
